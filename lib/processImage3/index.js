...
const processFile = function (FL, output_dir) {
  ...
  return BPromise.resolve()
    ...
    .then(function () {
      ...
      return gm(FL.file_path)
        ...
        .then(function () {
          ...
          all_versions.push(FL.getOriginalFileFinalName(null)); // [0] is the optimized but not converted one
    ...
    .then(function () {
      ...
      return BPromise.map(FL.versions, function (version) {
        ...
        if (version.height === 400 && (FL.file_size_kb <= 70.0)) {
          ...
          return gm(FL.file_path)
            ...
            .then(function () {
              ...
              all_versions.push(FL.getOriginalFileFinalName(version.height));
...
module.exports = function (ImageFunctions) {
  ...
  /**
   *
   * @param {object} process_image_opts
   * @param {string} process_image_opts.dir
   * @param {string} process_image_opts.output_dir
   * @param {object[]} process_image_opts.versions - array of objects, @height(number), @width(number|null)
   * @returns {*} - an array of all the file paths
   */
  ImageFunctions.prototype.processImage3 = function (process_image_opts) {
    ...
    const valid = ajv.validate(schema, process_image_opts);
    ...
    if (!path.isAbsolute(process_image_opts.dir)) {
      throw new Error('Path to image folder has to be absolute');
    }
    
    if (!path.isAbsolute(process_image_opts.output_dir)) {
    ...
    return BPromise.resolve()
      .then(function () {
        ...
        return fs.readdirAsync(process_image_opts.dir)
          .then(function (d) {
            return d;
          });
      ...
      .map(function (file) {
        ...
        // exclude folders
        if (!fs.statSync(path.join(process_image_opts.dir, file)).isFile()) {
          return true;
        }
        ...
        const FL = new OriginalFile(process_image_opts.dir, base_name, ext_name, process_image_opts.versions);
        
        return processFile(FL, process_image_opts.output_dir)
          .then(function (d) {
